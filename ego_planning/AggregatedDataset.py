    ...
    def __getitem__(self, index: int) -> dict:
        ...
        if len(agent_indices):
            ...
            for v_index in agent_indices:
                data_agent = self.prediction_dataset[v_index]

                out_net = self.prediction_model(torch.from_numpy(data_agent["image"]).unsqueeze(0).to(self.device))
        ...
        data_ego['image'] = np.append(data_ego['image'], prediction_dimension, axis = 0)
        return data_ego
...
class AggregatedDataset(BaseEgoDataset):
    def __init__(
            self,
            cfg: dict,
            zarr_dataset: ChunkedDataset,
        ...
        self.device = device
        super().__init__(cfg, zarr_dataset, rasterizer)
    ...
    def get_scene_dataset(self, scene_index: int) -> "AggregatedDataset":
        """
        Returns another EgoDataset dataset where the underlying data can be modified.
        This is possible because, even if it supports the same interface, this dataset is np.ndarray based.
        ...
        """
        dataset = self.dataset.get_scene_dataset(scene_index)
        return AggregatedDataset(self.cfg, dataset, self.rasterizer, self.perturbation, self.prediction_model)
